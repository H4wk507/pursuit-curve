...
\subsubsection{Uniwersalność równania pościgu bezpośredniego}

Równanie pościgu bezpośredniego ma identyczną postać dla dowolnego wymiaru $n \geq 2$:
\begin{equation}
\frac{d\vec{p}}{dt} = v_p \cdot \frac{\vec{t} - \vec{p}}{\|\vec{t} - \vec{p}\|_2}, \quad \vec{p}, \vec{t} \in \mathbb{R}^n
\end{equation}

Ta uniwersalność wynika z faktu, że:
\begin{itemize}
    \item Norma euklidesowa $\|\cdot\|_2$ naturalnie generalizuje się na dowolny wymiar
    \item Wektor jednostkowy $(\vec{t} - \vec{p})/\|\vec{t} - \vec{p}\|_2$ zawsze wskazuje kierunek do celu
    \item Nie ma potrzeby modyfikacji algorytmu przy zmianie wymiaru przestrzeni
\end{itemize}

\subsubsection{Zachowanie warunku zbieżności}

Warunek konieczny przechwycenia $v_p > v_t$ jest \textbf{niezmienniczy względem wymiaru}. Niezależnie od tego, czy pościg odbywa się w $\mathbb{R}^2$ czy $\mathbb{R}^{20}$, ścigający musi być szybszy od celu, aby zagwarantować przechwycenie.

Weryfikacja numeryczna dla $n \leq 20$ potwierdziła, że:
\begin{itemize}
    \item Trajektorie w wyższych wymiarach zachowują te same właściwości jakościowe co w 2D/3D
    \item Czas przechwycenia zależy wyłącznie od geometrii problemu (odległości, prędkości, trajektorii celu), nie od wymiaru przestrzeni otaczającej
\end{itemize}
...
\begin{itemize}
    \item Równanie pościgu bezpośredniego jest \textbf{uniwersalne} - identyczna formuła dla $\mathbb{R}^2, \ldots, \mathbb{R}^n$
