...
def calculate_torus_values():
    ...
    for ratio in ratios:
        ...
        strategy = ContinuousDirectPursuitTorus(
            vel=pursuer_speed, target_strategy=target_strategy, R=R, r=r
        )
        
        try:
            solution = run_continuous_simulation(
                initial_state, strategy, t_span=(0, 50), max_step=0.02
            )
            ...
            if catch_time >= 49.9:
                print(f"  Czas pościgu: >50 s (brak przechwycenia)")
...
def calculate_torus_values_v2():
    ...
    for v_p in speeds:
        ...
        target_strategy = ContinuousTargetTorusStrategy(omega_u=omega_u, omega_v=omega_v)
        strategy = ContinuousDirectPursuitTorus(
            vel=v_p, target_strategy=target_strategy, R=R, r=r
        )
        
        try:
            solution = run_continuous_simulation(
                initial_state, strategy, t_span=(0, 30), max_step=0.02
            )
            catch_time = solution.t[-1]
